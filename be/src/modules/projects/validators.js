...
export const projectValidator = [
    ...
    body('description').optional({ checkFalsy: true }).matches(descriptionRegex).withMessage('Project description length not sufficient'),
    body('status').optional({ checkFalsy: true }).isIn(statusValues).withMessage('Invalid status value.'),
    ...
    body('cover_image_url').optional({ checkFalsy: true }).matches(urlRegex).withMessage('Invalid cover_image_url.'),
    body('icon_url').optional({ checkFalsy: true }).matches(urlRegex).withMessage('Invalid icon_url.'),
    body('tags').optional({ checkFalsy: true }).isArray({ max: 10 }).withMessage('A maximum of 10 tags is allowed.'),
