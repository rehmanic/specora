...
function HistogramChart({ data, currencySymbol }) {
  ...
  useEffect(() => {
    ...
    async function render() {
      const { Chart, BarController, BarElement, CategoryScale, LinearScale, Tooltip, Title } = await import("chart.js");
      ...
      chartRef.current = new Chart(canvasRef.current, {
        type: "bar",
        data: {
          labels: data.map((b) => `${currencySymbol}${b.binStart.toLocaleString()}`),
          datasets: [
            {
              label: "Frequency",
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                ...
                label: (item) => `Count: ${item.raw} (${(data[item.dataIndex].frequency * 100).toFixed(1)}%)`,
              },
            },
          },
          scales: {
...
function SCurveChart({ data, currencySymbol }) {
  ...
  useEffect(() => {
    let cancelled = false;
    async function render() {
      const { Chart, LineController, LineElement, PointElement, CategoryScale, LinearScale, Tooltip, Title, Filler } =
...
export default function Page() {
  ...
  const handleSaveEstimates = async () => {
    ...
    const toSave = Object.entries(estimates)
      .filter(([, est]) => est.optimistic_hours !== "" && est.most_likely_hours !== "" && est.pessimistic_hours !== "")
      .map(([reqId, est]) => ({
          ...
          {loading ? (
            ...
            <Tabs defaultValue="setup" orientation="vertical" className="flex flex-col gap-6 md:flex-row">
              ...
              <div className="flex-1">
                {/* ─── Setup Tab ────────────────────────── */}
                <TabsContent value="setup" className="animate-fade-in mt-0 space-y-6 outline-none">
                  <Card className="border-border/50 shadow-sm">
                    <CardHeader className="bg-muted/30 border-border/50 border-b">
                      ...
                      <CardDescription>Set the base economic parameters for this project.</CardDescription>
                    </CardHeader>
                ...
                {/* ─── Estimates Tab ────────────────────── */}
                <TabsContent value="estimates" className="animate-fade-in mt-0 space-y-6 outline-none">
                  <div className="bg-muted/30 border-border/50 flex items-center justify-between rounded-xl border px-5 py-4">
                    <div>
                      ...
                      <CardDescription className="mt-1">
                        Enter three-point estimates (in hours) for each requirement: Optimistic, Most Likely, and
                        Pessimistic.
                      </CardDescription>
                    </div>
                    ...
                    <CardContent className="p-0">
                      {requirements.length === 0 ? (
                        <div className="p-12 text-center">
                          ...
                          <p className="text-muted-foreground/80 text-sm">Add requirements to this project first.</p>
                        </div>
                      ) : (
                        <Table>
                          <TableHeader>
                                ...
                                <TableRow key={req.id}>
                                      ...
                                      <TooltipProvider>
                                        <Tooltip>
                                          <TooltipTrigger asChild>
                                            <p className="text-muted-foreground mt-0.5 line-clamp-1 cursor-help text-sm">
                                  ...
                                  <TableCell>
                                    <Input
                                      ...
                                      onChange={(e) => handleEstimateChange(req.id, "optimistic_hours", e.target.value)}
                                    />
                                  </TableCell>
                                  <TableCell>
                                    <Input
                                      ...
                                      onChange={(e) =>
                                        handleEstimateChange(req.id, "most_likely_hours", e.target.value)
                                      }
                                    />
                                  </TableCell>
                                  <TableCell>
                                    <Input
                                      ...
                                      onChange={(e) =>
                                        handleEstimateChange(req.id, "pessimistic_hours", e.target.value)
                                      }
                                    />
                                  </TableCell>
                                </TableRow>
                ...
                {/* ─── Results Tab ──────────────────────── */}
                <TabsContent value="results" className="animate-fade-in mt-0 space-y-6 outline-none">
                  {/* Simulate Button */}
                  <Card className="border-border/50 shadow-sm">
                    <CardContent className="py-6">
                      <div className="flex items-center justify-between">
                        ...
                        <Button onClick={handleSimulate} disabled={simulating} size="lg" className="gap-2">
                          {simulating ? (
                            <>
                  ...
                  {!results ? (
                    <Card className="border-border/50 shadow-sm">
                      <CardContent className="py-16 text-center">
                        ...
                        <h3 className="font-display mb-2 text-xl font-semibold">No Results Yet</h3>
                        <p className="text-muted-foreground mx-auto max-w-md">
                          Configure your project settings, enter estimates, and run the simulation to see probabilistic
                    ...
                    <>
                      ...
                      <div className="text-muted-foreground flex items-center gap-2 text-sm">
                        ...
                        <Badge variant="outline">{results.config.iterations.toLocaleString()} iterations</Badge>
                        <Badge variant="outline">
                          {currencySymbol}
                          {results.config.hourly_rate}/hr
                        </Badge>
                        <Badge variant="outline">{results.config.num_developers} developer(s)</Badge>
                      </div>

                      {/* Cost Stats */}
                      <div>
                        <h3 className="mb-3 flex items-center gap-2 text-lg font-semibold">
                          <DollarSign className="text-primary h-4 w-4" /> Cost Analysis
                        </h3>
                        <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
                          {[
                            {
                              label: "Mean Cost",
                              value: formatCost(results.cost.mean),
                              icon: TrendingUp,
                              color: "text-primary",
                            },
                            {
                              label: "Median (P50)",
                              value: formatCost(results.cost.p50),
                              icon: BarChart3,
                              color: "text-blue-500",
                            },
                            {
                              label: "P80 Cost",
                              value: formatCost(results.cost.p80),
                              icon: TrendingUp,
                              color: "text-amber-500",
                            },
                            {
                              label: "P95 Cost",
                              value: formatCost(results.cost.p95),
                              icon: AlertTriangle,
                              color: "text-red-500",
                            },
                            {
                              label: "Std Dev",
                              value: formatCost(results.cost.stdDev),
                              icon: BarChart3,
                            ...
                            <Card key={stat.label} className="border-border/50 shadow-sm">
                              <CardContent className="flex items-center gap-3 p-4">
                                <div className={`bg-muted/50 rounded-lg p-2 ${stat.color}`}>
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
                          {[
                            {
                              label: "Mean Duration",
                              value: formatHours(results.duration.mean),
                              icon: Clock,
                              color: "text-primary",
                            },
                            {
                              label: "Median (P50)",
                              value: formatHours(results.duration.p50),
                              icon: BarChart3,
                              color: "text-blue-500",
                            },
                            {
                              label: "P80 Duration",
                              value: formatHours(results.duration.p80),
                              icon: TrendingUp,
                              color: "text-amber-500",
                            },
                            {
                              label: "P95 Duration",
                              value: formatHours(results.duration.p95),
                              icon: AlertTriangle,
                              color: "text-red-500",
                            },
                            {
                              label: "Std Dev",
                              value: formatHours(results.duration.stdDev),
                              icon: BarChart3,
                              color: "text-muted-foreground",
                            },
                          ].map((stat) => (
                      ...
                      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                        <Card className="border-border/50 shadow-sm">
                          <CardContent className="pt-6">
                            <div className="h-[300px]">
                              <HistogramChart data={results.cost.histogram} currencySymbol={currencySymbol} />
                            </div>
                          </CardContent>
                        </Card>
                        ...
                        <Card className="border-border/50 shadow-sm">
                          <CardContent className="pt-6">
                            <div className="h-[300px]">
                              <SCurveChart data={results.cost.cumulative} currencySymbol={currencySymbol} />
                            </div>
                          </CardContent>
                        </Card>
                        ...
                        <Card className="border-border/50 shadow-sm lg:col-span-2">
                          <CardContent className="pt-6">
                            <div className="h-[300px]">
                              <DurationHistogramChart data={results.duration.histogram} />
                            </div>
                          </CardContent>
                        </Card>
                      </div>
                          ...
                          <Table>
                            ...
                            <TableBody>
                              {[
                                {
                                  level: "80%",
                                  costLow: results.cost.p10,
                                  costHigh: results.cost.p90,
                                  durLow: results.duration.p10,
                                  durHigh: results.duration.p90,
                                },
                                {
                                  level: "90%",
                                  costLow: results.cost.p10,
                                  costHigh: results.cost.p95,
                                  durLow: results.duration.p10,
                                  durHigh: results.duration.p95,
                                },
                                {
                                  level: "Most Likely (P50)",
                                  costLow: results.cost.p50,
                                  costHigh: null,
                                  durLow: results.duration.p50,
                                  durHigh: null,
                                },
                              ].map((row) => (
