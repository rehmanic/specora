  ...
  return (
    <ProtectedRoute requiredPermissions={["view_group_chat_messages"]}>
      <div className="bg-background relative z-0 flex h-full flex-col">
        ...
        {/* Messages Area */}
        <div className="min-h-0 flex-1 overflow-hidden scroll-smooth bg-transparent">
          <ScrollArea className="h-full w-full">
            <div className="space-y-6 p-6">
              {loading && messages.length === 0 ? (
                ...
                <div className="text-muted-foreground flex h-full items-center justify-center pt-10">
                  No messages yet. Start the conversation!
                </div>
              ) : (
                messages.map((msg, index) => (
                  <div key={msg.id || index} className="animate-fade-in" style={{ animationDelay: `${index * 0.05}s` }}>
                    <Message
                      id={msg.id}
                      text={msg.content}
                      timestamp={
        ...
        <div className="bg-transparent p-4 pb-6 sm:p-6">
          <div className="shadow-primary/5 border-border bg-card/80 mx-auto max-w-4xl scroll-smooth rounded-2xl border shadow-2xl ring-1 ring-black/5 backdrop-blur-xl transition-all dark:ring-white/5">
